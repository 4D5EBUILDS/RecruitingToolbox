...
window.SECTION_DEFS = [
  ...
  { id:"enlistment", title:"Enlistment Documents", short:"Enlistment", reg:"UR 601-210 · AR 601-210", cond:A, items:[
    { id:"umf680",    cond:A, init:"pending",  label:"UMF 680-3A — Request for Examination",
      sub:"Initial personal-data / processing form — verify every item vs. GENESIS; dates in YYYYMMDD",
      help:h("UMF 680-3A — Request for Examination","USMEPCOM Form 680-3A Instr (Feb 2025)","UMF 680-3A is the initial form collecting each applicant's personal data (SSN, legal name, address, citizenship, sex, education tier code, ASVAB/aptitude and medical-exam requests) and is submitted prior to any processing at MEPS or MET sites. It is NOT the medical prescreen — the prescreen is the DD 2807-2 / UMF 680-3A-2. Verify every item against the USMIRS/GENESIS record.\n\nName (Item 2): full legal name, last-first-middle, one space between fields; no nicknames or shortened names. A middle initial is allowed only when the initial is part of the legal name. Dates use YYYYMMDD (DOB Item 10, last full medical exam, previous test dates). An applicant younger than 17 is not eligible for processing. Education (Item 12) is years-completed plus the credential/tier code.\n\nSignatures: no applicant signature is required on the 680-3A during USMIRS 1.1 MVP (Item 21). Applicant certification signatures still apply at MEPS/MET (Item 22) and in the recruiter's presence (Item 24f); recruiting personnel certify Item 25. The Privacy Act Statement must be printed on the reverse.\n\nCorrections: no whiteout or correction tape — strike the error with a single line, enter the correct data, and the applicant initials AND dates the correction. Updates to Item A (SPF), Item 3 (Current Address), Item 4 (HOR), and Items 17a/17b (Recruiter/Station ID) do NOT require applicant initials — only the sponsoring Service representative's initials and the date.",null,["Whiteout / correction tape — NOT ACCEPTABLE (strike, initial, date)","Nickname or shortened legal name — NOT AUTHORIZED","Dates not in YYYYMMDD format — fix before submission","Data does not match USMIRS / GENESIS — QRP return"]) },
