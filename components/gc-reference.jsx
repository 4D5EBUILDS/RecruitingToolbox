...
const GC_STEPS = [
  ...
  { n:7,  text:"Cross-check all dates across DD 2807-2, UMF 680-3A (Request for Examination — personal-data form, not the medical prescreen), and GENESIS — any date mismatch between these sources is an automatic return. DOB, POB, and exam/treatment dates must match exactly across all three. Dates on the UMF 680-3A are entered in YYYYMMDD format." },
  { n:8,  text:"Verify DD 369 returns — all three jurisdictions (city, county, state) must be in hand before projection. Each must be dated within 6 months. Each must be run separately under every alias the applicant has used — IAW UM 21-022, hand-jamming an alias onto an already-returned form is not acceptable. Check that forms are addressed to law enforcement agencies and NOT courthouses." },
  ...
  { n:12, text:"Prior Service & Reserve separations: Effective May 17, 2025 (IAW USAREC MSG 26-016), National Guard and Reserve discharges must provide BOTH a DD Form 214 and a DD Form 214-1 (Reserve Component Addendum). NGB 22 is deprecated. If separating in grade E-5 or above, verify the respective RA/AR Grade Determination Worksheet is complete, including TPU Unit Acceptance Letters with paragraph, line, and position numbers." },
  { n:13, text:"\"No Medical Required\" (B0M0) Auditing: Verify the Periodic Health Assessment (PHA) is green/amber (within 15 months of MEPS processing). Ensure MEDPROS IMR shows a valid HIV test within 2 years. Ensure a Unit Commander MFR is present for any RED IMR categories or non-deployable PHA items. Verify GC entered the verbal health statement in DD 1966 remarks. Ensure NO USMIRS prescreen (DD 2807-2) was submitted, as doing so permanently negates B0M0 processing (USMEPCOM 40-1 para 2-11c)." },
  { n:14, text:"Moral waiver packet review (always check — confirm a waiver was not incorrectly initiated for a non-waiver offense):\n• UF 601-210.08: offenses listed newest to oldest; only waived offense(s) asterisked; AFQT on form matches current RZ score\n• DD Form 370: three references present (employment, school, personal); dates of attendance/employment match RZ exactly; no family members as personal references; college/vo-tech references include transcript\n• Court dockets: all three components per offense — charging document + court finding/sentencing + final disposition\n• DD 369: covers every jurisdiction where an offense occurred, within 6 months, run under all aliases\n• FL 601-210.04: present if applicant was confined 24+ hours in any institution; front page institution address and recruiter signature complete; institutional official signs and dates\n• DA Form 3072-2: all income entries are monthly — not annual, not weekly. Do not include anticipated military pay\n• Marijuana/paraphernalia possession or use: confirm NO waiver packet was initiated — AR 601-210 para 4-6 (Mar 2026) removed the waiver requirement. If a packet was opened, it must be withdrawn. Verify offense classification (possession/use vs. distribution) with GC before assuming." },
];
...
const GC_RETURNS = [
  ...
  { type:"nogo",     text:'DD 369 addressed to Courthouse instead of Police Department, Sheriff Department, or State Department — automatic return (IAW USAREC checklist).' },
  { type:"nogo",     text:'FL 601-210.04 missing when applicant was confined 24+ hours in any institution (jail, detention, juvenile facility, inpatient program) — institution must fill out and sign page 2. Must verify front page recruiter signature and institution address are complete.' },
  ...
  { type:"nogo",     text:'Company Commander MFR has a copy-paste subject line error (e.g. "SUBJECT: Assumption of Command" instead of self-ID or waiver recommendation) — RETURN.' },
  ...
  { type:"category", text:"Minor Consent & FCP Failures" },
  { type:"nogo",     text:"Minor Consent: Parent or Recruiter signed before the applicant (DD 2807-2 or DD 1966) — applicant signature date must be prior to or on the same date as parental and recruiter signatures." },
  { type:"nogo",     text:"Minor Consent: Parent signature missing from Section II or Section VI of the DD Form 2807-2." },
  { type:"nogo",     text:"DA Form 5305 Family Care Plan for Reserve sole parent signed by unauthorized representative — TPU Unit Commander must sign/approve; responsibility cannot be delegated to readiness NCO or recruiter." },
  { type:"nogo",     text:"Reserve separation post-May 17, 2025: NGB 22 or discharge orders provided instead of the required DD Form 214 and DD Form 214-1 — RETURN." },
];
